...
/**
 * REST controller for managing {@link my.company.app.domain.Pedidos}.
 */
@RestController
...
public class PedidosResource {
     ...
     */
    @PostMapping("")
    public ResponseEntity<Pedidos> createPedidos(@RequestBody Pedidos pedidos) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/pedidos/" + pedidos.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, pedidos.getId().toString()))
            .body(pedidos);
    }
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<Pedidos> updatePedidos(@PathVariable(value = "id", required = false) final Long id, @RequestBody Pedidos pedidos)
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, pedidos.getId().toString()))
            .body(pedidos);
    }
    ...
    public ResponseEntity<Pedidos> partialUpdatePedidos(
        ...
        Optional<Pedidos> result = pedidosRepository
            ...
            .map(existingPedidos -> {
                if (pedidos.getDireccion() != null) {
                    existingPedidos.setDireccion(pedidos.getDireccion());
                }
                if (pedidos.getFechaAlta() != null) {
                    existingPedidos.setFechaAlta(pedidos.getFechaAlta());
                }
                if (pedidos.getFechaEntrega() != null) {
                    existingPedidos.setFechaEntrega(pedidos.getFechaEntrega());
                }
                if (pedidos.getUsername() != null) {
                    existingPedidos.setUsername(pedidos.getUsername());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, pedidos.getId().toString())
        );
    }
